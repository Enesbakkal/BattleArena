...
- **`web/src/api/charactersApi.ts`:** `createCharacter` sends **POST** `/api/characters` with a **camelCase** JSON body aligned with `CreateCharacterRequest` (`name`, `universe`, `rarity`, `baseAttack`, `baseDefense`, `baseSpeed`, optional `biography` / `imageUrl`).
- **`web/src/components/CharactersGrid.tsx`:** an **Add character** form, **`useMutation`** for the POST, and **`invalidateQueries`** on `['characters']` after success so the grid refetches.
- **`CharactersGrid.css`:** layout/styles for the form.
...
- **Frontend:** `web/` uses Vite + React + TanStack Table for GET and a create form for POST (see Batch 4).
...
**What the page does:** `GET /api/characters?page=&pageSize=` with Previous/Next, page-size select, and Refresh. **Add character** form posts to **`POST /api/characters`** (camelCase body); on success the grid refetches. Empty database still shows the empty-row message until at least one POST succeeds (or you insert rows another way; see `BattleArena.Api.http`).
