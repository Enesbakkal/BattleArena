...
- **Fix (scoped to the app shell + grid):** **`App.css`** — **`main`** gets **`text-align: left`** and a darker default text color; **`CharactersGrid.css`** sets explicit dark text, stronger table header styling, visible **View / Edit / Delete** buttons (tinted backgrounds), clearer inputs and pager/toolbar controls, and a styled error box with **`white-space: pre-line`** for multi-field messages. **`Modal.css`** darkens dialog text and improves primary/secondary button contrast.
...
- **Frontend:** modal CRUD + TanStack Query (**Batch 5**); friendly **`400`** text + contrast tweaks (**Batch 6**).
...
**What the page does:** Paged **`GET /api/characters`** with Previous/Next, page size, Refresh. **Modals** for **create** (**`POST`**), **read** (**`GET`** by id), **update** (**`PUT`**), **delete** (**`DELETE`**). TanStack Query **`useQuery`** for list + detail; **`useMutation`** + **`invalidateQueries`** after writes (**Batch 5**). **Validation / API errors** shown as readable lines, not raw JSON (**Batch 6**). Empty DB shows the empty-row message until a row exists.
