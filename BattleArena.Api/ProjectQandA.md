...
## Batch 4 (session notes — empty grid, dev ports, cancel exceptions, create UI)

### 1. GET `/api/characters` returns JSON but `items` is empty — is the API wrong?

- Usually **no**. An empty `items` array and `totalCount: 0` mean there are **no rows** in `Characters` yet. Add rows with **POST** `/api/characters` (from the web form, `BattleArena.Api.http`, or any HTTP client), then GET will show data.

### 2. Browser says connection failed to `http://127.0.0.1:51` (or similar)

- Vite’s default dev server port is **5173**, not `51`. Open **`http://127.0.0.1:5173`** or **`http://localhost:5173`** (the URL `npm run dev` prints).

### 3. Debugger shows `TaskCanceledException` / “task was canceled” while the page looks OK

- In **development**, React **Strict Mode** can mount components twice; the first `fetch` may be **aborted** when the tree remounts. TanStack Query’s second request often succeeds, so you see a normal grid (or empty state) without a user-visible error. A **first-chance** cancel in the debugger is not always a product bug.

### 4. Should I use `localhost` or `127.0.0.1` for the SPA and API base URL?

- **CORS** allows both `http://localhost:5173` and `http://127.0.0.1:5173` (and HTTPS variants) in `Program.cs`. For fewer surprises, keep **`VITE_API_BASE_URL`** and the **address you type in the browser** on the **same host style** (both localhost or both 127.0.0.1).

### 5. What did we add on the frontend for “add record”?

- **Superseded by Batch 5:** the first version used an **inline** form; we later moved **create/read/update/delete** into **modals** and expanded the API (GET by id, PUT, DELETE). See **Batch 5** for the current design.

---

## Batch 5 (session — full character CRUD, modals, mutations, reuse reminder)

### 1. What we added on the backend (characters)

- **`Character.Update(...)`** on the domain entity so catalog fields change in one place; **`UpdateCharacterCommand`** loads the entity tracked, calls **`Update`**, **`SaveChangesAsync`**.
- **`GetCharacterByIdQuery`** → **`CharacterDetailDto`** (includes **`biography`** and full fields for read/edit). **`404`** when id is missing.
- **`UpdateCharacterCommand`** + FluentValidation (same rules as create, plus non-empty **`Id`**). Returns **`bool`**; API maps **false** → **404**, success → **204 No Content**.
- **`DeleteCharacterCommand`** removes the row; **404** if missing, **204** on success.
- **`CharactersController`:** **`GET /api/characters/{id}`**, **`PUT /api/characters/{id}`** (body same shape as **`CreateCharacterRequest`**), **`DELETE /api/characters/{id}`**.
- **`BattleArena.Api.http`:** examples for GET/PUT/DELETE (set **`@characterId`**).

### 2. What we added on the frontend (`web/`)

- **`Modal.tsx` + `Modal.css`:** reusable dialog (backdrop click, **Escape**, title, footer slot).
- **`CharacterFormFields.tsx`:** shared fields + **`emptyCharacterForm`**, **`detailToForm`**, **`valuesToBody`** for create/edit.
- **`charactersApi.ts`:** **`fetchCharacterById`**, **`updateCharacter`**, **`deleteCharacter`**; **`types/characters.ts`:** **`CharacterDetail`**.
- **`CharactersGrid.tsx`:** **Add character** opens **create** modal; each row has **View / Edit / Delete** (read, edit, confirm-delete modals). **`useQuery`** for the paged list and for **detail** when a read/edit modal is open (shared **`queryKey`** pattern **`['character', id]`**).

### 3. Mutations — what they are and where we use them

- **TanStack Query** splits server state into **queries** (read, cacheable, refetch) and **mutations** (write: POST/PUT/DELETE).
- **`useMutation`** wraps an async function that **changes data on the server**. It exposes **`mutate` / `mutateAsync`**, **`isPending`**, **`isError`**, **`error`**, **`isSuccess`**, etc., so the UI can disable buttons and show messages without manual `useState` for every request.
- **Why invalidate after success?** The grid’s **`useQuery`** cache still holds the **old** list until you **invalidate** (or update the cache by hand). We call **`queryClient.invalidateQueries({ queryKey: ['characters'] })`** in **`onSuccess`** so the paged list refetches and stays consistent with SQL.
- **Where in this project:** all three live in **`CharactersGrid.tsx`**:
  - **`createMutation`** — **`createCharacter(valuesToBody(form))`** → on success invalidate **`['characters']`**, close modal.
  - **`updateMutation`** — **`updateCharacter(id, body)`** → invalidate **`['characters']`** and **`['character', id]`** (detail cache), close modal.
  - **`deleteMutation`** — **`deleteCharacter(id)`** → invalidate **`['characters']`**, close modal.
- **Reads** use **`useQuery`** (paged list + single character for modals), not **`useMutation`**.

### 4. Reminder — generic grids and modals (`<T>`) for reuse

- **Today:** **`CharactersGrid`**, **`CharacterFormFields`**, and **`CharacterRow`** are **typed specifically** for characters. That is fine for one screen; it does not scale if we add many similar admin pages (players, items, seasons, …).
- **Future direction (team reminder):** extract **generic building blocks** so **one implementation** powers every list/detail screen:
  - **`DataGrid<T>`** (or similar): accepts **column definitions**, **paged query** hook or **`queryKey` + fetcher**, **pagination state**, optional **row actions** factory **`(row: T) => …`**.
  - **`EntityModal<T>`** / **`useCrudModals<T>`**: open **create | read | update | delete** with shared **backdrop, focus, Escape**, and pluggable **form** / **read-only** / **confirm** content.
  - **Shared hooks:** e.g. **`useInvalidateListQuery(queryKey)`** after any mutation so **invalidate** logic is not copy-pasted per page.
- **Benefit:** add a feature once (**export CSV**, **bulk delete**, **keyboard nav**, **optimistic updates**) and **all** grids/modals inherit it. **Cost:** more abstraction and TypeScript generics discipline (`T` must carry **`id`**, DTO shapes, etc.).
- **Practical approach:** keep **Characters** as the reference implementation until patterns stabilize; then extract **`Modal`**-level pieces first (already started), then grid, then a thin **CRUD template** per resource (API module + column map + form component).

---

## Batch 6 (session — validation UX + readable UI)

### 1. Why did users see raw JSON for validation errors?

- The API returns **400** with **Problem Details** JSON (`errors`, `title`, `type`, …). Throwing that string into **`Error.message`** showed the whole blob in the UI.
- **`web/src/api/formatApiError.ts`** parses JSON when the body starts with **`{`**, walks **`errors`**, maps API property names to short labels (**Rarity → “Rarity (must be 1–5)”**, **BaseAttack → ATK**, …), strips the leading **`'PropertyName'`** from FluentValidation text, and joins lines. **`charactersApi.ts`** uses **`formatApiErrorBody`** on every failed **`fetch`** so modals and the grid show plain sentences instead of JSON.

### 2. Why did headers, buttons, and table text look “washed out”?

- **`index.css`** sets **`color: var(--text)`** on **`:root`** (a muted gray). Children inherit it, so table cells and action buttons looked low-contrast; **`#root`** also used **`text-align: center`**, which affected layout.
- **Fix (contrast):** **`CharactersGrid.css`** sets explicit dark text, stronger table headers, visible **View / Edit / Delete** buttons (tinted backgrounds), clearer inputs and pager/toolbar, and a styled error box with **`white-space: pre-line`**. **`Modal.css`** darkens dialog text and improves primary/secondary button contrast. Early on, **`App.css`** / **`main`** also set **`text-align: left`**; full chrome later moved to **`AppLayout`** (**Batch 7**), with **`#root`** full width for the shell.

---

## Batch 7 (session — app shell, React Router, env badge, learning doc)

### 1. Responsive layout shell (`AppLayout`)

- **`web/src/components/AppLayout.tsx` + `AppLayout.css`:** **Header** (dark bar), **sidebar** (navigation), **main** (page content), **footer** (API base URL hint).
- **Mobile (`< 960px`):** **hamburger** toggles a **fixed** sidebar that slides in from the left; a **scrim** (full-area dim button) closes it; **Escape** closes the menu. **`matchMedia('(min-width: 960px)')`** syncs **`isDesktop`** and clears the open state when crossing to desktop.
- **Desktop (`≥ 960px`):** sidebar is a **grid column** (persistent); hamburger and scrim hidden. **`aria-expanded` / `aria-controls` / `aria-hidden`** used for basic accessibility.
- **`useLocation`:** whenever **`pathname`** changes, the sidebar **closes** on mobile (after **`NavLink`** navigation).

### 2. Client-side routing (`react-router-dom`)

- **Dependency:** **`react-router-dom`** in **`web/package.json`**.
- **`main.tsx`:** **`BrowserRouter`** wraps **`App`** (inside **`QueryClientProvider`** so route-level components can still use **`useQuery`**).
- **`App.tsx`:** **`Routes`** / **`Route`** — layout route with **`element={<AppLayout />}`** and **no path**; children use **`Outlet`** inside **`AppLayout`**. **`index`** → **`Navigate to="/characters"`** (**`replace`**). **`path="characters"`** → **`CharactersGrid`**. **`path="*"`** → **`NotFoundPage`** (`web/src/pages/`).
- **Sidebar:** **`NavLink to="/characters"`** with **`end`** and **`className`** callback for active styles. **Brand** in the header is also a **`NavLink`** to **`/characters`**.

### 3. Environment badge and root layout CSS

- **`EnvironmentBadge.tsx` + `.css`:** shows **DEV** vs **PROD** from **`import.meta.env.PROD`** (Vite); **`title`** tooltip includes **`MODE`**. Placed in the **header** (right, after a flex spacer).
- **`index.css` — `#root`:** changed from a **fixed max width** centered column to **`width: 100%`** so the **shell** can use the full viewport (needed for responsive header/sidebar).
- **`App.css`:** minimal placeholder after chrome moved to **`AppLayout`**.

### 4. Teaching doc (7 × ~40 minutes)

- **`web/REACT-WEEK-COURSE.md`:** day-by-day plan covering **Vite/TS entry**, **routing**, **shell/responsive/env**, **TanStack Query**, **Table + modals + forms**, **API + `formatApiError`**, plus a week-end **checklist**. Complements **`REACT-LEARNING.md`** (broader file list) with a **time-boxed** reading path.
...
- **GET** `.../api/characters/{id}` — **`CharacterDetailDto`** (includes **`biography`**); **404** if missing.
- **POST** `.../api/characters` — body **`CreateCharacterRequest`**; **201** with new id.
- **PUT** `.../api/characters/{id}` — same body as POST; **204** on success, **404** if missing.
- **DELETE** `.../api/characters/{id}` — **204** on success, **404** if missing.
- **Paged response:** `PagedCharacterRowsResult` — **`items`** + **`totalCount`**.
- **Frontend:** modal CRUD + TanStack Query (**Batch 5**); friendly **`400`** text + contrast (**Batch 6**); **`AppLayout`** + **`react-router-dom`** + **DEV/PROD** badge (**Batch 7**).
...
**Stack:** `@tanstack/react-table` for the grid, `@tanstack/react-query` for server-state and caching, **`react-router-dom`** for routes and **`NavLink`** / **`Outlet`** layout.
...
**CORS:** `Program.cs` allows the Vite dev origins on both **localhost** and **127.0.0.1** for ports **5173** (HTTP and HTTPS).

**What the app does:** **`AppLayout`** — header (brand + **DEV/PROD** badge), **sidebar** (**Characters** + placeholder), **main** (**`<Outlet />`**), footer. **`/`** redirects to **`/characters`**; unknown paths → **404** page. Paged **`GET /api/characters`** with Previous/Next, page size, Refresh. **Modals** for **create** (**`POST`**), **read** (**`GET`** by id), **update** (**`PUT`**), **delete** (**`DELETE`**). TanStack Query **`useQuery`** / **`useMutation`** + **`invalidateQueries`** (**Batch 5**). **Validation / API errors** readable (**Batch 6**). Empty DB shows the empty-row message until a row exists.

**Longer React walkthroughs:** `web/REACT-LEARNING.md` (broad file tour) · **`web/REACT-WEEK-COURSE.md`** (7-day ~40 min/day plan, **Batch 7**).
